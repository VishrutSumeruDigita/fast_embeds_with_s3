...
def download_images_from_s3(bucket_name, prefix, local_dir):
    """
    Download images from an S3 prefix into a local directory
    """
    print(f"Downloading from {bucket_name}/{prefix} into {local_dir}")
    s3 = boto3.client('s3')
    ...
    print(f"Done prefix {prefix}")

def main():
    load_dotenv()
    bucket = os.getenv("S3_BUCKET")
    if not bucket:
        print("Set S3_BUCKET in .env file")
        return
    base = "test_images"
    for prefix in PREFIXES:
        local_path = os.path.join(base, prefix)
        download_images_from_s3(bucket, prefix, local_path)
